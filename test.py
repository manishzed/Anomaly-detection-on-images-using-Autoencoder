...
if cfg.weight_file:
    ...
    mod_ =r"C:\Users\hp\Desktop\priyasoftweb\chip_img_anomaly_autoencoder\chip_v8\chip_anomaly_v5\results\type_1\chechpoints\ssim_loss\4165-0.03160.hdf5"
    autoencoder.load_weights(mod_)
...
def get_threshold(cfg):
    ...
    valid_good_list = glob(cfg.train_data_dir + '/*bmp')
...
def get_results(file_list, cfg):
    for img_path in file_list:
        ...
        cv2.imwrite(cfg.save_dir+'/'+c+'_'+img_name+'_residual_1.bmp', mask)
        cv2.imwrite(cfg.save_dir+'/'+c+'_'+img_name+'_origin_1.bmp', test_img)
        cv2.imwrite(cfg.save_dir+'/'+c+'_'+img_name+'_rec_1.bmp', rec_img)
        cv2.imwrite(cfg.save_dir+'/'+c+'_'+img_name+'_visual_1.bmp', vis_img)
